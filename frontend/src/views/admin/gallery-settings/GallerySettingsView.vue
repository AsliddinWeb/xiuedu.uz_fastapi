...
import { LayoutPanelLeft, FolderOpen, Image as ImageIcon } from 'lucide-vue-next'
...
const tabs = [
  ...
  { key: 'categories', label: 'Kategoriyalar',    icon: FolderOpen,      component: CategoriesTab },
  { key: 'items',      label: 'Rasmlar',          icon: ImageIcon,       component: ItemsTab }
]
...
<template>
  <div>
    <div class="mb-6">
      <h1 class="text-xl font-display font-bold text-ink-dark dark:text-white">Galereya sahifasi</h1>
      <p class="text-sm text-ink-faint mt-0.5">Kategoriyalar va rasmlarni boshqarish</p>
    </div>
    ...
    <div class="grid lg:grid-cols-[220px_1fr] gap-6">
      <aside class="lg:sticky lg:top-4 self-start">
        <nav class="rounded-2xl bg-white dark:bg-slate-800 border border-surface-muted dark:border-slate-700 p-2 space-y-0.5">
          <button
            v-for="t in tabs" :key="t.key" type="button" @click="activeKey = t.key"
            :class="['w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition-colors text-left',
                     activeKey === t.key ? 'bg-primary-50 text-primary-800 font-semibold dark:bg-primary-900/40 dark:text-white'
