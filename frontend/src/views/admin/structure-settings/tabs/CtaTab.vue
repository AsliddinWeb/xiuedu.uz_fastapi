...
<template>
  ...
  <div v-else>
    <div class="flex items-center justify-between mb-6">
      <div>
        <h2 class="text-lg font-display font-bold text-ink-dark dark:text-white">Pastki CTA</h2>
        <p class="text-xs text-ink-faint mt-0.5">Sahifa pastidagi qora gradient banner</p>
      </div>
    ...
    <div class="rounded-2xl bg-white dark:bg-slate-800 border border-surface-muted dark:border-slate-700 p-5 mb-5 space-y-4">
      ...
      <div>
        ...
        <input type="text" v-model="form.cta_primary_url" placeholder="/contact"
               class="w-full h-10 px-3 rounded-lg border border-surface-muted dark:border-slate-700 bg-white dark:bg-slate-900 text-sm" />
      </div>
    </div>

    <div class="rounded-2xl bg-white dark:bg-slate-800 border border-surface-muted dark:border-slate-700 p-5 mb-5 space-y-4">
      <p class="text-xs font-bold uppercase tracking-wider text-ink-faint">Sekundar CTA tugma</p>
      <MultilingualInput v-model="form" base="cta_secondary_label" label="Tugma matni" />
      <div>
        <label class="block text-sm font-medium text-ink-medium mb-1.5">URL</label>
        <input type="text" v-model="form.cta_secondary_url" placeholder="/about"
               class="w-full h-10 px-3 rounded-lg border border-surface-muted dark:border-slate-700 bg-white dark:bg-slate-900 text-sm" />
      </div>
    </div>
